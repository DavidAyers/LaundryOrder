...
final class Ocean_Sticky_Header {
	...
	public function scripts() {
		...
		$local_variables = array(
			'stickyChoose' 				=> get_theme_mod( 'osh_sticky_choose', 'auto' ),
			'stickyStyle' 				=> get_theme_mod( 'osh_sticky_header_style', 'shrink' ),
			'shrinkLogoHeight' 			=> apply_filters( 'osh_shrink_header_logo_height', get_theme_mod( 'osh_shrink_header_logo_height', '30' ) ),
			'stickyEffect' 				=> get_theme_mod( 'osh_sticky_header_effect', 'none' ),
			'hasStickyTopBar' 			=> $this->if_topbar_sticky(),
			'hasStickyMobile' 			=> get_theme_mod( 'osh_has_sticky_mobile', false ),
		);
		...
		// Localize script
		wp_localize_script( 'osh-js-scripts', 'oshLocalize', $local_variables );
	...
	public function head_css( $output ) {
		...
		if ( 'top' != $headerStyle && 'fixed' != get_theme_mod( 'osh_sticky_header_style', 'shrink' ) ) {
			...
			if ( ! empty( $shrink_height ) && '54' != $shrink_height ) {
				$css .= '.is-sticky #site-header.shrink-header #site-logo #site-logo-inner, .is-sticky #site-header.shrink-header .oceanwp-social-menu .social-menu-inner, .is-sticky #site-header.shrink-header.full_screen-header .menu-bar-inner{height:'. $shrink_height .'px;}';
				$css .= '.is-sticky #site-header.shrink-header #site-navigation-wrap .dropdown-menu > li > a, .is-sticky #site-header.shrink-header #oceanwp-mobile-menu-icon a{line-height:'. $shrink_height .'px;}';
			}
